...
<script>
  export default {
	...
	methods:{
		getDetail(type){
			this.$http.get('chapterrementushu/detail/' + this.id, {}).then(res => {
				if (res.data.code == 0) {
		...
		catalogueClick(){
			this.$http.get('rementushu/detail/' + this.detail.refid, {}).then(res => {
				if (res.data.code == 0) {
					this.$router.push({path: '/index/rementushuDetail', query: {detailObj: JSON.stringify(res.data.data)}});
				}
			});
		},
		nextClick(){
