...
<div :style='{"width":"1100px","padding":"20px","boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="80px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="图书名称" prop="tushumingcheng">
            <el-input v-model="ruleForm.tushumingcheng" 
                placeholder="图书名称" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}'  label="图书分类" prop="tushufenlei">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="图书封面" v-if="type!='cross' || (type=='cross' && !ro.tushufengmian)" prop="tushufengmian">
            <file-upload
            tip="点击上传图书封面"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="图书封面" prop="tushufengmian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="图书作者" prop="tushuzuozhe">
            <el-input v-model="ruleForm.tushuzuozhe" 
                placeholder="图书作者" clearable ></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="上架日期" prop="shangjiariqi" disabled>
              <el-date-picker
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="图书介绍" prop="tushujieshao">
            <editor 
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info(id) {
        this.$http.get('rementushu/detail/${id}', {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
                  ...
                  params: params
                     ...
                     if(res.data.data.total>=crossoptnum) {
                         ...
                         // 跨表计算
                          ...
                          this.$http.post('rementushu/add', this.ruleForm).then(res => {
                              if (res.data.code == 0) {
                                  this.$message({
                                      message: '操作成功',
                                      type: 'success',
