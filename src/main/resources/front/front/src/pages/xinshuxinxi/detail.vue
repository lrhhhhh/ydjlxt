	...
	<div class="detail-preview" :style='{"width":"1200px","margin":"10px auto","position":"relative"}'>
		<div class="attr" :style='{"padding":"20px","position":"relative","flexWrap":"wrap","background":"#f5f5f5","display":"flex"}'>
			...
			<div class="info" :style='{"padding":"10px","boxShadow":" 0px 4px 10px 0px rgba(0,0,0,0.302)","margin":"0 10px 0 10px","borderRadius":"20px","alignItems":"center","flexDirection":"column","flex":"1","background":"#ffff","display":"flex","width":"800px"}'>
				...
				<div class="item" :style='{"width":"80%","padding":"10px","margin":"0 0 10px 0","justifyContent":"spaceBetween","display":"flex"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#999","textAlign":"right","width":"110px","fontSize":"14px","lineHeight":"40px","height":"40px"}'>发布日期</div>
					<div  :style='{"padding":"0 10px","color":"#666","background":"#e8f9ff","width":"100%","fontSize":"14px","lineHeight":"40px","height":"40px"}'>{{detail.faburiqi}}</div>
				</div>
...
<script>
  ...
  export default {
    ...
    methods: {
        ...
        freeReadClick(){
            this.$router.push(('/index/xinshuxinxiChapter?id=' + this.freeChapterList[0].id))
        },
        chapterClick(vipread,id){
            ...
            this.$router.push(('/index/xinshuxinxiChapter?id=' + id))
        },
        getFreeChapterList() {
      ...
      storeup(type) {
        ...
        if (type == -1 && this.isStoreup) {
          this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 1, refid: this.detail.id, tablename: 'xinshuxinxi', userid: localStorage.getItem('userid')}}).then(res => {
            if (res.data.code == 0 && res.data.data.list.length > 0) {
              this.isStoreup = true;
      ...
      thumbsupOrCrazily(type) {
        ...
        this.$http.post('xinshuxinxi/update', this.detail).then(res => {});
      },
    ...
    getThumbsupOrCrazilyStatus() {
        if(localStorage.getItem("Token")) {
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 21, refid: this.detail.id, tablename: 'xinshuxinxi', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
            ...
            this.$http.get('storeup/list', {params: {page: 1, limit: 1, type: 22, refid: this.detail.id, tablename: 'xinshuxinxi', userid: localStorage.getItem('userid')}}).then(res => {
              if (res.data.code == 0 && res.data.data.list.length > 0) {
      ...
      getDiscussList(page) {
        this.$http.get('discussxinshuxinxi/list', {params: {page, limit: this.pageSize, refid: this.detail.id}}).then(res => {
          if (res.data.code == 0) {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$http.post('discussxinshuxinxi/add', this.form).then(res => {
              if (res.data.code == 0) {
                this.form.content = '';
                this.getDiscussList(1);
                this.$message({
