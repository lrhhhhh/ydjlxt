...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"10px 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>笔记名称</div>
        <el-input v-model="formSearch.bijimingcheng" placeholder="笔记名称" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>笔记类型</div>
        <el-select v-model="formSearch.bijileixing" placeholder="请选择笔记类型" :clearable="true">
          <el-option v-for="(item, index) in bijileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>笔记日期</div>
        <el-date-picker 
          v-model="timeRange"
          type="daterange"
		  :style='{"border":"2px solid #2E61E1","padding":"3px 10px","outline":"none","borderRadius":"8px","background":"#fff","width":"auto","justifyContent":"center"}'
          range-separator="至"
          start-placeholder="笔记日期起始"
          end-placeholder="笔记日期结束"
	  ...
	  <el-button v-if="isAuth('yuedubiji','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#00b9ea","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/yuedubijiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
	...
	<div class="list" :style='{"width":"950px","margin":"0 0 10px","flex":"1","order":"4"}'>
		...
		<div class="list1 index-pv1" :style='{"width":"100%","padding":"0 10px","flex":"1","height":"auto"}'>
			<div :style='{"boxShadow":"0px 4px 10px 0px rgba(0,0,0,0.302)","padding":"10px","margin":"10px 1%","textAlign":"center","background":"#fff","display":"inline-block","width":"23%","position":"relative","height":"auto"}' v-for="(item, index) in dataList" :key="index" @click="toDetail(item)" class="list-item animation-box">
				<img :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-if="item.tupianxinxi && item.tupianxinxi.substr(0,4)=='http'" :src="item.tupianxinxi" class="image" />
				<img :style='{"width":"100%","objectFit":"cover","display":"block","height":"275px"}' v-else :src="baseUrl + (item.tupianxinxi?item.tupianxinxi.split(',')[0]:'')" class="image" />
				...
				<div :style='{"padding":"0 10px","lineHeight":"30px","fontSize":"14px","color":"rgba(0, 0, 0, 1)"}' class="name ">{{item.bijimingcheng}}</div>
			</div>
		</div>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/bijileixing/bijileixing').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.bijimingcheng != '') searchWhere.bijimingcheng = '%' + this.formSearch.bijimingcheng + '%';
        if (this.formSearch.bijileixing != '') searchWhere.bijileixing = this.formSearch.bijileixing;
        if (this.timeRange.length > 0) {
          searchWhere.bijiriqistart = this.timeRange[0];
          searchWhere.bijiriqiend = this.timeRange[1];
        }
        params['sort'] = 'bijiriqi';
        params['order'] = 'desc';
        if (this.curFenlei != '全部') searchWhere.bijileixing = this.curFenlei;
        this.$http.get('yuedubiji/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/yuedubijiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
