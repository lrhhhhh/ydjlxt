<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="章节数" prop="chapternum">
					<el-input v-model="ruleForm.chapternum" placeholder="章节数" clearable  :readonly="ro.chapternum"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="章节数" prop="chapternum">
					<el-input v-model="ruleForm.chapternum" placeholder="章节数" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="章节标题" prop="chaptertitle">
					<el-input v-model="ruleForm.chaptertitle" placeholder="章节标题" clearable  :readonly="ro.chaptertitle"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="章节标题" prop="chaptertitle">
					<el-input v-model="ruleForm.chaptertitle" placeholder="章节标题" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="章节内容" prop="content">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.content" label="章节内容" prop="content">
...
export default {
    ...
    info(id) {
      this.$http({
        url: `chapterxinshuxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
        ...
        this.ruleForm.content = this.ruleForm.content.replace(reg,'../../../springbootr2ork/upload');
        } else {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `chapterxinshuxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.chapterxinshuxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
