<template>
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px","background":"url(http://codegen.caihongy.cn/20230401/bd196cf1072f48afae5676066a72e297.jpg) fixed no-repeat center top / 100% 100%"}' style="width: 100%;">
		...
		>
			<template >
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="笔记名称" prop="bijimingcheng">
					<el-input v-model="ruleForm.bijimingcheng" placeholder="笔记名称" clearable  :readonly="ro.bijimingcheng"></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="笔记名称" prop="bijimingcheng">
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="select" v-if="type!='info'"  label="笔记类型" prop="bijileixing">
					<el-select :disabled="ro.bijileixing" v-model="ruleForm.bijileixing" placeholder="请选择笔记类型" >
						<el-option
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else class="input" label="笔记类型" prop="bijileixing">
					<el-input v-model="ruleForm.bijileixing"
						placeholder="笔记类型" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="笔记日期" prop="bijiriqi">
					<el-date-picker
						format="yyyy 年 MM 月 dd 日"
						value-format="yyyy-MM-dd"
						v-model="ruleForm.bijiriqi" 
						type="date"
						:readonly="ro.bijiriqi"
						placeholder="笔记日期"
					></el-date-picker> 
				</el-form-item>
				<el-form-item :style='{"margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.bijiriqi" label="笔记日期" prop="bijiriqi">
					<el-input v-model="ruleForm.bijiriqi" placeholder="笔记日期" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-if="type!='info'"  label="笔记内容" prop="bijineirong">
					<editor 
				...
				<el-form-item :style='{"margin":"0 0 20px 0"}' v-else-if="ruleForm.bijineirong" label="笔记内容" prop="bijineirong">
                    <span :style='{"fontSize":"16px","lineHeight":"40px","color":"#666","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.bijineirong"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.bijineirong = this.ruleForm.bijineirong.replace(reg,'../../../springbootr2ork/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `yuedubiji/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.yuedubijiCrossAddOrUpdateFlag = false;
						 this.parent.search();
